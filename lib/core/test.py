...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.MODEL.BODY_UV_ON and boxes.shape[0] > 0:
        ...
        cls_bodys = im_detect_body_uv(model, im_scale, boxes, blob_conv)
...
def im_detect_body_uv(model, im_scale, boxes, blob_conv):
    ...
    U_uv, V_uv, Index_UV, AnnIndex = model.module.body_uv_net(blob_conv, inputs)
